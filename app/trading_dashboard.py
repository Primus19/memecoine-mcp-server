...
<nav class="nav" id="nav"><button class="active" data-view="overview">Overview</button><button data-view="coinbase">Coinbase</button><button data-view="forex">Forex / OANDA</button><button data-view="solana">Solana</button><button data-view="market">Market Scanner</button><button data-view="performance">Model Performance</button><button data-view="intelligence">Intelligence Ledger</button><button data-view="operations">Operations</button></nav>
...
<section id="intelligence" class="view"><div class="grid metrics" id="intelMetrics"></div><div class="notice section">Append-only evidence. Insights cannot modify live rules or promote a strategy automatically.</div><div class="grid two section"><article class="card"><h2>Saved strategy intelligence</h2><div class="table-wrap" id="intelStrategies"></div></article><article class="card"><h2>Evidence-backed tips and hypotheses</h2><div class="timeline" id="intelLearnings"></div></article></div><article class="card section"><h2>Recently captured evidence</h2><div class="table-wrap" id="intelEvidence"></div></article></section>
...
function positions(){const out=[],cb=S.coinbase||{},p=cb.portfolio?.open_position;if(p)out.push(['Coinbase',p.product_id,p.status,val(p,'net_qty')??p.fills?.net_qty,val(p,'entry_price'),val(p,'mark_price'),p.net_unrealized_pnl_usdc]);const fx=S.forex||{},broker=fx.broker||fx.reconciliation||fx.account||{};for(const x of arr(fx.open_positions||broker.open_trades||broker.positions))out.push(['OANDA',val(x,'instrument','symbol','pair'),val(x,'side')||(n(x.units)>0?'BUY':'SELL'),val(x,'units','quantity'),val(x,'entry_price','price'),val(x,'mark_price','current_price'),val(x,'unrealized_pl','unrealized_pnl_usd')]);for(const x of arr((S.solana||{}).paperPositions))out.push(['Solana paper',val(x,'symbol','mint'),'PAPER',x.quantity,x.entryUsd,val(x,'markUsd','markPrice'),n(val(x,'markUsd','markPrice'))-n(x.entryUsd)]);return out}
function renderOverview(){const cb=S.coinbase||{},ctrl=cb.portfolio?.controls||{},fx=S.forex||{},broker=fx.broker||fx.account||fx.reconciliation||{},sol=S.solana||{},fxNav=val(broker,'NAV','nav')??fx.nav,fxPnl=val(broker,'realized_pl')??fx.realized_pnl_usd,cbVal=ctrl.equity_usdc??cb.permitted_capital_usdc;const ps=positions(),up=Object.values(S.sources||{}).filter(x=>x.ok).length,total=Object.keys(S.sources||{}).length;$('metrics').innerHTML=metric('Coinbase equity',money(cbVal),`${money(cb.realized_pnl_usdc)} realized`,cls(cb.realized_pnl_usdc))+metric('OANDA NAV',money(fxNav),`${money(fxPnl)} realized`,cls(fxPnl))+metric('Solana paper P&L',money(sol.paperRealizedPnlUsd),`${n(sol.paperObservations)} closed observations`,cls(sol.paperRealizedPnlUsd))+metric('Open positions',ps.length,'Across all systems')+metric('Scanner candidates',arr(S.discovery?.candidates).length,`${arr(S.discovery?.candidates).filter(x=>x.qualified||x.paper_qualified).length} qualified`)+metric('Service feeds',`${up}/${total}`,up===total?'All responding':'Attention required',up===total?'good':'bad');$('positions').innerHTML=table(['System','Asset','Side','Qty','Entry','Mark','Unrealized P&L'],ps.map(r=>r.map((x,i)=>i===6?`<b class="${cls(x)}">${money(x)} · NOT FINAL</b>`:esc(x))));$('readiness').innerHTML=Object.entries(S.sources||{}).map(([k,v])=>kv(k.toUpperCase(),badge(v.ok?'ONLINE':'FAILED',v.ok?'good':'bad'))).join('')+kv('Coinbase mode',badge(cb.mode,cb.mode?.includes('LIVE')?'good':'warn'))+kv('Forex mode',badge(fx.mode,fx.mode?.includes('LIVE')?'good':'warn'))+kv('Solana live',badge(sol.live?'ARMED':'DISABLED',sol.live?'warn':''));const acts=[...arr(sol.paperFills).slice(0,5),...arr(fx.recentActions).slice(0,5)].sort((a,b)=>Date.parse(val(b,'at','time','closed_at')||0)-Date.parse(val(a,'at','time','closed_at')||0)).slice(0,8);$('activity').innerHTML=acts.length?acts.map(x=>event(`${x.action||x.type||'ACTION'} ${x.symbol||x.instrument||''}`,x.reason||x.entryReason||`${x.strategy||''} ${x.realizedPnlUsd!==undefined?money(x.realizedPnlUsd):''}`,val(x,'at','time','closed_at'))).join(''):'<div class="empty">No recent confirmed or paper trade actions.</div>';const problems=[];for(const [k,v] of Object.entries(S.sources||{}))if(!v.ok)problems.push(event(`${k} feed failed`,v.error||'No data returned','',true));for(const x of arr(sol.blockers))problems.push(event('Solana blocker',x,'',true));if(fx.last_error)problems.push(event('Forex error',fx.last_error,fx.last_scan,true));$('attention').innerHTML=problems.join('')||event('No critical dashboard-source failure','All configured read-only feeds are responding.');}
...
function render(){renderOverview();renderCoinbase();renderForex();renderSolana();renderMarket();renderPerformance();renderIntelligence();renderOps();$('updated').textContent='Updated '+new Date(S.generated_at).toLocaleTimeString();$('updated').classList.add('flash');setTimeout(()=>$('updated').classList.remove('flash'),800)}
