...
<nav class="nav" id="nav"><button class="active" data-view="overview">Overview</button><button data-view="coinbase">Coinbase</button><button data-view="forex">Forex / OANDA</button><button data-view="solana">Solana</button><button data-view="market">Market Scanner</button><button data-view="performance">Model Performance</button><button data-view="operations">Operations</button></nav>
...
function event(title,detail,time='',bad=false){return `<div class="event ${bad?'errorbox':''}"><b>${esc(title)}</b><div>${esc(detail)}</div>${time?`<div class="time">${when(time)}</div>`:''}</div>`}
...
function render(){renderOverview();renderCoinbase();renderForex();renderSolana();renderMarket();renderPerformance();renderOps();$('updated').textContent='Updated '+new Date(S.generated_at).toLocaleTimeString();$('updated').classList.add('flash');setTimeout(()=>$('updated').classList.remove('flash'),800)}
