...
def render_forex_report(report: dict) -> str:
    ...
    broker_rows = [{"id": x.get("trade_id"), "pair": x.get("instrument"), "side": x.get("side"),
                    ...
                    "stop": x.get("stop_price"), "target": x.get("target_price"),
                    "pnl": _money(x.get("unrealized_pnl_usd")), "financing": _money(x.get("financing_usd"))}
...
<div style="background:#e0f2fe;border:2px solid #38bdf8;border-radius:12px;padding:18px;margin-bottom:14px;overflow-x:auto"><h2 style="color:#075985">Current live broker positions</h2><table style="border-collapse:collapse;width:100%;font-size:13px"><tr><th>Trade ID</th><th>Pair</th><th>Side</th><th>Units</th><th>Entry</th><th>Executable now</th><th>Stop</th><th>Target</th><th>Unrealized</th><th>Financing</th></tr>{rows(broker_rows, [("id","ID"),("pair","Pair"),("side","Side"),("units","Units"),("entry","Entry"),("current","Current"),("stop","Stop"),("target","Target"),("pnl","P&L"),("financing","Financing")], background="#f0f9ff")}</table></div>
