...
def render_forex_report(report: dict) -> str:
    ...
    if "V5" in str(five.get("version") or ""):
        promotion_note = (f'{int(five_promotion.get("closed_observations") or 0)} / '
                          f'{int(five_promotion.get("required_closed_observations") or 30)} closed V5 observations; '
                          'positive cost-stressed expectancy is required. Historical V2–V4 trades are excluded.')
    else:
        promotion_note = (f'{int(five_promotion.get("profitable_closes_observed") or 0)} / '
                          f'{int(five_promotion.get("required_additional_profitable_closes") or 2)} profitable closes. '
                          'Net P&amp;L must remain positive. Historical V2/V3 trades are excluded.')
    five_trades = five.get("trades") or []
    broker_trades = report.get("broker_open_trades") or []
    checkpoints = report.get("trade_checkpoints") or []
    live_checkpoints = report.get("live_trade_checkpoints") or []
    ...
    broker_rows = [{"id": x.get("trade_id"), "pair": x.get("instrument"), "side": x.get("side"),
                    ...
                    "slippage": (f'{float(x["entry_slippage_bps"]):+.2f} bps'
                                 if x.get("entry_slippage_bps") is not None else "Unavailable"),
                    "commission": _money(x.get("commission_usd")),
    ...
    live_checkpoint_rows = [{"id": x.get("trade_id"), "pair": x.get("instrument"),
                             "minute": f"{x.get('checkpoint_minutes')}m",
                             "time": x.get("observed_at"), "price": x.get("executable_price"),
                             "pnl": _money(x.get("pnl_usd")),
                             "spread": f'{float(x.get("spread_bps") or 0):.2f} bps',
                             "source": x.get("source_observed_at")}
                            for x in live_checkpoints]
    learning_rows = [{"id": x.get("trade_id"), "pair": x.get("instrument"),
                      ...
                      "action": "EXIT REQUESTED" if x.get("execute") else
                                "WOULD EXIT" if x.get("would_exit_now") else "MONITOR"}
...
<div style="background:#e0f2fe;border:2px solid #38bdf8;border-radius:12px;padding:18px;margin-bottom:14px;overflow-x:auto"><h2 style="color:#075985">Current live broker positions</h2><p><b>Amount put</b> is notional market exposure. Because Forex is leveraged, estimated margin and maximum planned loss are separate.</p><table style="border-collapse:collapse;width:100%;font-size:13px"><tr><th>Trade ID</th><th>Pair</th><th>Side</th><th>Units</th><th>Entry</th><th>Executable now</th><th>Amount put / exposure</th><th>Est. margin</th><th>Max risk</th><th>Stop</th><th>Target</th><th>Unrealized</th><th>Return on amount</th><th>Entry slippage</th><th>Commission</th><th>Financing</th></tr>{rows(broker_rows, [("id","ID"),("pair","Pair"),("side","Side"),("units","Units"),("entry","Entry"),("current","Current"),("amount","Amount"),("margin","Margin"),("risk","Risk"),("stop","Stop"),("target","Target"),("pnl","P&L"),("return","Return"),("slippage","Slippage"),("commission","Commission"),("financing","Financing")], background="#f0f9ff")}</table></div>
<div style="background:#f5f3ff;border:2px solid #8b5cf6;border-radius:12px;padding:18px;margin-bottom:14px;overflow-x:auto"><h2 style="color:#6d28d9">Cross-strategy learning • Active profit retention</h2><p><b>Mode:</b> GUARDED LIVE. The planned target remains active while the thesis is aligned. A close is requested only after a protected ratchet floor is breached, or after at least +0.10R MFE, a 40% giveback, a still-profitable executable quote, and confirmed multi-horizon deterioration. Counterfactual alternatives remain SHADOW ONLY until their adoption gates are met.</p><table style="border-collapse:collapse;width:100%;font-size:13px"><tr><th>Trade ID</th><th>Pair</th><th>Current</th><th>Peak</th><th>Protected floor</th><th>Action</th></tr>{rows(learning_rows, [("id","ID"),("pair","Pair"),("current","Current"),("peak","Peak"),("floor","Floor"),("action","Action")], background="#faf5ff")}</table></div>
...
<div style="background:#eff6ff;border-radius:12px;padding:18px;margin-bottom:14px;overflow-x:auto"><h2>Live-trade executable checkpoints</h2><table style="border-collapse:collapse;width:100%;font-size:13px"><tr><th>Trade</th><th>Pair</th><th>Checkpoint</th><th>Recorded</th><th>Executable price</th><th>P&amp;L</th><th>Spread</th><th>Source time</th></tr>{rows(live_checkpoint_rows, [("id","Trade"),("pair","Pair"),("minute","Checkpoint"),("time","Recorded"),("price","Price"),("pnl","P&L"),("spread","Spread"),("source","Source")])}</table></div>
