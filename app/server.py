...
from .consolidated_learnings import consolidated_report
...
LIVE_ARMED=(os.getenv("LIVE_TRADING","false").lower()=="true" and
            os.getenv("LIVE_CONFIRMATION","")=="I_ACCEPT_THE_25_USDC_LIVE_RISK" and
            os.getenv("PROFITABILITY_GATE_PASSED","false").lower()=="true")
...
def intelligence_snapshot()->dict:
    ...
    snapshot["intelligence"]=consolidated_report()
    return snapshot
...
async def intelligence_report(request):
    ...
    return JSONResponse(consolidated_report(),headers={
        "Cache-Control":"no-store, max-age=0",
        "X-Content-Type-Options":"nosniff",
        "Referrer-Policy":"no-referrer",
    })
