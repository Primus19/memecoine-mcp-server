...
class ForexExecutorTests(unittest.TestCase):
    def test_trend_continuation_is_independent_and_paper_only(self):
        ...
        self.assertEqual("2026-09-04T12:00:00Z", signals[0]["signal_closed_at"])
        self.assertEqual(0, signals[0]["evaluation_latency_seconds"])

    def test_trend_continuation_rejects_missing_or_stale_completed_candle(self):
        base = {
            "symbol": "EUR_USD", "calendar_verified": True,
            "economic_event_source": "https://official.example/calendar",
            "market_veto": False, "high_impact_calendar_blackout": False,
        ...
        with self.assertRaisesRegex(Exception, "completed M5"):
            trend_continuation_signals(base)
        with self.assertRaisesRegex(Exception, "signal is stale"):
            trend_continuation_signals({**base,
